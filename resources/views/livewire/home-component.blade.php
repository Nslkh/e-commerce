	...
	<div class="container">
			...
			<div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container " data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}'>
				...
				<div class="product product-style-2 equal-elem ">
					...
					<a href="{{route('product.details',['slug'=>$sproduct->slug])}}" class="product-name"><span>{{$sproduct->name}}</span></a>
						<div class="wrap-price"><ins><p class="product_price">{{$sproduct->regular_price}}</p></ins> <del><p class="product-price">{{$sproduct->sale_price}}</p></del></div>
					</div>
				</div>
				@endforeach
			</div>
                ...
                <div class="product product-style-2 equal-elem ">
                  <div class="product-thumnail">
                    <a href="{{route('product.details',['slug'=>$latestProduct->slug])}}" title="{{$latestProduct->name}}">
                      <figure><img src="{{ asset('assets/images/products') }}/{{$latestProduct->image}}" width="800" height="800" alt="{{$latestProduct->name}}"></figure>
                    </a>
                  </div>
                  <div class="product-info">
                    <a href="{{route('product.details',['slug'=>$latestProduct->slug])}}" class="product-name"><span>{{$latestProduct->name}}</span></a>
		...
		<div class="wrap-show-advance-info-box style-1">
			...
			<div class="wrap-products">
				...
				<div class="tab-contents">
					@foreach ($categories as $key=>$category )
					<div class="tab-content-item {{$key==0 ? 'active' : ''}} " id="{{$category->id}}">
						<div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
               			@php 
